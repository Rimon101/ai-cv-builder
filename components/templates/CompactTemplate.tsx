...
export function CompactTemplate({ data }: { data: CVData }) {
  return (
    <div
      id="cv-preview-content"
      style={{ fontFamily: data.fontFamily }}
      className="mx-auto min-h-[1123px] w-[794px] bg-white px-10 py-8 text-slate-900"
    >
      <header className="flex items-start justify-between gap-6 border-b pb-4">
        ...
        {data.personal.photo && (
          ...
          <img src={data.personal.photo} alt={data.personal.fullName} className="h-16 w-16 rounded-full object-cover" />
        )}
      </header>

      <div className="mt-4 grid grid-cols-[1fr_240px] gap-6">
        <main>
          {data.summary && (
            <section>
              <h2 className="text-sm font-semibold" style={{ color: data.accentColor }}>
                Summary
              </h2>
