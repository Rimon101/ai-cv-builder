...
export function ProfessionalTemplate({ data }: { data: CVData }) {
  return (
    ...
    >
      <header className="flex items-start justify-between gap-6 border-b-2 pb-4" style={{ borderColor: data.accentColor }}>
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Professional CV</p>
        ...
        {data.personal.photo && (
          ...
          <img src={data.personal.photo} alt={data.personal.fullName} className="h-20 w-20 rounded-lg object-cover" />
        )}
      </header>

      <section className="mt-6 grid grid-cols-[1fr_220px] gap-6">
        <main>
          {data.summary && (
            <div>
